...
x.setAttribute("id", "videoWrapper");
// x.setAttribute("srcdoc", '<video id="my-player"></video><script src="JS/1.0.0.js"><\/script>');
document.getElementById("videoContainer").appendChild(x);

var lastClicked = "";

var myVar = document.getElementById("myScript").getAttribute("data-my_var_1");
...
var abc = document.createElement("link");
abc.type = "text/css";
abc.rel = "stylesheet";
abc.href = "CSS/test.css";
...
document.addEventListener("PClick", (e) => {
  if (lastClicked == e.detail.clickname) {
    HideAll();
  }
...
function playerEvents(Player, videoPlayerContainer, settings) {
  ...
  document.addEventListener("start", () => {
    ...
    // get the time of the pause and convert it into milisections and match with speed
    var currentTime =
      (Math.floor(1800 * (1000 / settings.FrameSpeed)) /
        (1000 / settings.FrameSpeed)) *
      1000;
...
async function loadShowtag(Player, videoPlayerContainer, ProgramCode) {
  ...
  var ProductsInProgram = false;
  var CelebritiesInProgram = true;

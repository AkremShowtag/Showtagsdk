...
var config = {
  studio: true,
  designer: true,
  platform: "androidTV", //androidTV web
  url: "https://tag2081.showtagenabled.com/programs/202007051209-2081/202007051209-2081.mp4",
  videoId: "202007051209-2081",
  width: "960", //px
  idUser: "testuser",
};

if (config.platform) {
  classtype = config.platform;

  console.log("classtype: " + classtype);

  if (classtype == "androidTV") {
    androidTV = true;

    console.log("androidTV: " + androidTV);
...
const ContentScraper = () => {
  ...
  for (var i = 0; i < ele.length; i++) {
    List =
      ...
      "content" +
      "-" +
      Math.floor(getOffset(document.getElementById(ele[i].id)).left / 10) +
      "-" +
...
const CustomProductScraper = () => {
  var ele = document.getElementsByClassName("MenuCellContainerCustomProducts");
  let List = "";
  for (var i = 0; i < ele.length; i++) {
    List =
      List +
...
const CelebritiesScraper = () => {
  var ele = document.getElementsByClassName("MenuCellContainerCelebrities");
  let List = "";
  for (var i = 0; i < ele.length; i++) {
    List =
      List +
...
const MusicScraper = () => {
  var ele = document.getElementsByClassName("MenuCellContainerMusic");
  let List = "";
  for (var i = 0; i < ele.length; i++) {
    List =
      List +
...
const VideoScraper = () => {
  var ele = document.getElementsByClassName("CelebrityVideoItemContainer");
  let List = "";
  for (var i = 0; i < ele.length; i++) {
    List =
      List +
...
const ScrapingButtons = () => {
  window.setTimeout(() => {
    var ele = document.getElementsByClassName("defaultMenuButton");
    let List = "";
    for (var i = 0; i < ele.length; i++) {
      List =
        List +
    ...
    JSProtocol.ButtonsList(List);
  }, 1000);
};

document.addEventListener("PClick", (e) => {
  if (e.detail.clickname == "Pause") {
    JSProtocol.isPaused(false);
  }
  if (
    e.detail.clickname == lastClicked &&
    e.detail.clickname.indexOf("vid") == -1
  ) {
    lastClicked = "";

    HideAll();
  } else {
    if (e.detail.clickname == "CloseIt") {
      document.getElementById("CelebrityVideoClose").click();
    }
    if (e.detail.clickname.indexOf("vid") != -1) {
      console.log("agaiiin");
      var arr = e.detail.clickname.split("vid");
      document.getElementById(arr[1]).click();
      JSProtocol.closeButton(true);
      setTimeout(() => {
        document.getElementById("CelebrityVideoClose").animate(
          [
            // key frames
    ...
    //Scaping content for the product button
    if (e.detail.clickname == "ButtonProduct") {
      setTimeout(ContentScraper, 1000);
      //Scraping content for celebrity button
    } else if (e.detail.clickname == "ButtonCelebrity") {
      setTimeout(CelebritiesScraper, 1000);
      //Scraping content for Custom button product
    } else if (e.detail.clickname == "ButtonCustomProduct") {
      setTimeout(CustomProductScraper, 1000);
      //Scraping content for music button
    } else if (e.detail.clickname == "ButtonMusic") {
      setTimeout(MusicScraper, 1000);
      //Scraping content for video button
    } else if (e.detail.clickname == "ButtonCelebrityVideoOverview") {
      setTimeout(VideoScraper, 1000);
    } else {
      // setTimeout(() => {
      //   console.log(
      //     '"QRCodeTag" + e.detail.clickname',
      //     "QRCodeTag" + e.detail.clickname
      //   );
      //   var elImg = document
      //     .getElementById("QRCodeTag" + e.detail.clickname)
      //     .querySelector("img");
      //   console.log("elImg", elImg);
      //   elImg.style.cssText = `width: 40px;
      //   height: 40px;
      //   padding: 0.5vw;
      //   background-color: #ffffff;
      //   border-radius: calc(8/(var(--baseWidth))*100vw);`;
      //   elImg.src = "sqfqf";
      // }, 1500);
    }
  }
...
document.addEventListener("showtag", (e) => {
  if (vs == undefined) {
    ...
    playerControl.src =
      "https://www.api.xs-anime.me/public/uploads/images/player.png";
    playerControl.style.position = "absolute";
    playerControl.style.top = "480px";
    playerControl.style.width = "960px";
    mainContainer.appendChild(playerControl);
...
document.addEventListener("customFocus", (e) => {
  // celebrity tag focus
  var focus = document.getElementById(e.detail.newFocus);
  var old = document.getElementById(e.detail.oldFocus);
  if (e.detail.newFocus.indexOf("celebrity_") != -1) {
    focus.style.backgroundImage = "url('./CSS/img/tag_celebrity_white.svg')";
    console.log("1");
  }
  if (e.detail.oldFocus.indexOf("celebrity_") != -1) {
    old.style.backgroundImage = "url('./CSS/img/tag_celebrity_yellowy.svg')";
    console.log("2");
  }
  if (e.detail.newFocus.indexOf("Button") != -1) {
    console.log("3");

    focus.style.backgroundColor = "#001ffd";
    focus.style.filter = "invert(1)";
  }
  if (e.detail.oldFocus.indexOf("Button") != -1) {
    console.log("4");

    old.style.backgroundColor = "#4E4E4E";
    old.style.filter = "invert(0)";
  }
  if (e.detail.newFocus.indexOf("content") != -1) {
    console.log("5");
    var tr = document.getElementById("ProductMenuContainer");
    var ils = tr.getElementsByClassName("MenuContainerScroll")[0];

    focus = document.getElementById(
      e.detail.newFocus.substr(0, e.detail.newFocus.indexOf("content"))
    );
    old = document.getElementById(
      e.detail.oldFocus.substr(0, e.detail.oldFocus.indexOf("content"))
    );
    focus.querySelector("div").style.cssText = `    opacity: 1;
    border: 2px solid var(--base-yellow);
    height: calc(391 / (var(--baseHeight)) * 100vh);`;
    if (
      parseInt(
        e.detail.newFocus.substr(0, e.detail.newFocus.indexOf("content"))
      ) >
      ...
      ils.scrollLeft = ils.scrollLeft + 200;
    } else if (
      parseInt(
        e.detail.newFocus.substr(0, e.detail.newFocus.indexOf("content"))
      ) <
      ...
      console.log("i am scrolling2");

      ils.scrollLeft = ils.scrollLeft - 200;
    }
  }
  if (e.detail.oldFocus.indexOf("content") != -1) {
    console.log("6");
    old = document.getElementById(
      e.detail.oldFocus.substr(0, e.detail.oldFocus.indexOf("content"))
    );
    old.querySelector(
      "div"
    ).style.cssText = `    margin-top: calc(37 / (var(--baseHeight)) * 100vh);
    width: calc(400 / (var(--baseWidth)) * 100vw);
    height: calc(191 / (var(--baseHeight)) * 100vh);
    border: 2px solid var(--menu-item-background);
    border-radius: calc(15 / (var(--baseWidth)) * 100vw);
    position: absolute;
    background-color: var(--menu-item-background);
    overflow: hidden;
    opacity: 1;`;
  }
  if (e.detail.newFocus.indexOf("celebrityScroll") != -1) {
    console.log("7");
    var tr = document.getElementById("CelebrityMenuContainer");
    var ils = tr.getElementsByClassName("MenuContainerScroll")[0];
    focus.querySelector("div").style.cssText = `    opacity: 1;
    border: 2px solid var(--base-yellow);
    height: calc(591 / (var(--baseHeight)) * 100vh);
    overflow: initial;`;
    var first = e.detail.newFocus.split("_");
    var second = e.detail.oldFocus.split("_");
    if (parseInt(first[1]) > parseInt(second[1])) {
      console.log("i am scrolling");
      ils.scrollLeft = ils.scrollLeft + 150;
  ...
  if (e.detail.oldFocus.indexOf("celebrityScroll") != -1) {
    console.log("8");
    old.querySelector(
      "div"
    ).style.cssText = `      margin-top: calc(37 / (var(--baseHeight)) * 100vh);
    width: calc(313 / (var(--baseWidth)) * 100vw);
    height: calc(210 / (var(--baseHeight)) * 100vh);
    border: 2px solid var(--menu-item-background);
    border-radius: calc(15 / (var(--baseWidth)) * 100vw);
    position: absolute;
    background-color: var(--menu-item-background);
  ...
  if (e.detail.newFocus.indexOf("customproduct") != -1) {
    console.log("9");
    focus.querySelector("div").style.cssText = `    opacity: 1;
    border: 2px solid var(--base-yellow);
    height: calc(391 / (var(--baseHeight)) * 100vh);`;
  }
  if (e.detail.oldFocus.indexOf("customproduct") != -1) {
    console.log("10");
    old.querySelector(
      "div"
    ).style.cssText = `    margin-top: calc(37 / (var(--baseHeight)) * 100vh);
  ...
  if (e.detail.newFocus.indexOf("Music_") != -1) {
    console.log("11");
    focus.querySelector("div").style.cssText = `    opacity: 1;
    border: 2px solid var(--base-yellow);
    height: calc(391 / (var(--baseHeight)) * 100vh);`;
  }
  if (e.detail.oldFocus.indexOf("Music_") != -1) {
    console.log("12");
    old.querySelector(
      "div"
    ).style.cssText = `    margin-top: calc(37 / (var(--baseHeight)) * 100vh);
  ...
  if (e.detail.newFocus.indexOf("vid") != -1) {
    console.log("12");

    console.log("hellsss");
    var arr = e.detail.newFocus.split("vid");

    focus = document.getElementById(arr[1]);
    focus.style.cssText = `
     border: 2px solid var(--base-yellow);
     `;
    focus.querySelector("div").style.backgroundImage =
      "url(CSS/img/icon_play_button_yellow.svg)";
  }
  if (e.detail.oldFocus.indexOf("vid") != -1) {
    console.log("13");
    var arr = e.detail.oldFocus.split("vid");
    old = document.getElementById(arr[1]);
    old.style.cssText = `
    margin-top: calc(37 / (var(--baseHeight)) * 100vh);
    width: calc(600 / (var(--baseWidth)) * 100vw);
    height: calc(191 / (var(--baseHeight)) * 100vh);
    border: 2px solid var(--menu-item-background);
    border-radius: calc(15 / (var(--baseWidth)) * 100vw);
    background-color: var(--menu-item-background);
    ...
    float: left;
    `;
    old.querySelector("div").style.backgroundImage =
      "url(CSS/img/icon_play_button.svg)";
  }
  if (e.detail.newFocus == "Pause") {
    playerControl.src =
      "https://www.api.xs-anime.me/public/uploads/images/player2.png";
  }
  if (e.detail.oldFocus == "Pause") {
    playerControl.src =
      "https://www.api.xs-anime.me/public/uploads/images/player.png";
  }
});
...
function playerEvents(Player, videoPlayerContainer, settings) {
  ...
  document.addEventListener("start", () => {
    ...
    if (settings.menuMinimize != 1) {
      ...
      // console.log("LoadShowtagEnabledMinimized");
      document.getElementById("showtagenabled").style.display = "block";
...
function adjustSizevideoPlayerContainer(videoCustomWidth) {
  ...
  if (videoCustomWidth) {
    // console.log("videoCustomWidth: " + videoCustomWidth);
...
function resetButtonHover() {
  // remove the class "Active" of any button in the menu
  var resetClass = document.getElementsByClassName("defaultMenuButton-Active");
...
function LoadShowtagEnabledMinimized(settings, currentTime) {
  ...
  document.getElementById("minimizeProductNotification").style.display = "none";

  // we update the text in the button with the default text
  document.getElementById("minimizeText").innerHTML = settings.menuMinimizeText;

  // console.log("showtag minimize functions loaded")
...
function DrawCelebrityContent(group, dataCvideo, settings) {
  ...
  if (settings.CelebrityVideoInProgram == 1) {
    ...
    //     var videoDetails = celebrityVideoArray.find(x => x.id_celebrityVideo == DivIdCelebrityVideo);
...
function CelebrityTags(currentTime, settings) {
  console.log("Celebrity Tags Fired");
...
function ShowCelebrityTags(Celebrities, settings, currentTime) {
  ...
  for (i = 0; i < Celebrities.length; i++) {
    ...
    if (
      ...
      console.log("celebrity hover default activated");
    ...
    if (settings.deactivateTagHoover == 1) {
      console.log("celebrity hover off activated");
    ...
    if (settings.deactivateProgramDetails == 1) {
      console.log("celebrity details activated");
  ...
  setTimeout(() => {
    for (i = 0; i < Celebrities.length; i++) {
      var { y, x } = getOffset(
        document.getElementById(Celebrities[i].id_celebrity)
      );
      Cel =
        Cel +
        "celebrity_" +
  ...
  if (androidTV == true) {
    ...
    for (i = 0; i < Celebrities.length; i++) {
      ...
      if (Celebrities[i].Instragram != null) {
        ...
        console.log("QRCodeInstagram" + Celebrities[i].id_celebrity);
  ...
  for (i = 0; i < ProductDetailsData.length; i++) {
    if (ProductDetailsData[i].ProductCustomGroup == false) {
      ...
      if (ProductDetailsData[i].Price != null) {
        productPrice =
          '<div class="ProductPrice">' + ProductDetailsData[i].Price + "</div>";
      }
  ...
  if (androidTV == true) {
    ...
    for (i = 0; i < ProductDetailsData.length - 1; i++) {
      ...
      if (ProductDetailsData[i].Url != null) {
        ...
        // where we need to capture the product id, find the URL and then redirect the user to the website
        new QRCode(
          document.getElementById("qrCode" + ProductDetailsData[i].id_product),
          "http://showtagenabled.com/" + ProductDetailsData[i].id_product
        );
      }
    }
  }
...
function ShowProductTags(Products, settings, currentTime) {
  ...
  // console.log("show brand logo: " + settings.tagShowBrandLogo);

  // console.log("deactivate product details: " + settings.deactivateProgramDetails);
  ...
  for (i = 0; i < Products.length; i++) {
    ...
    // console.log("product x: " + Products[i].x)
  ...
  for (i = 0; i < Products.length; i++) {
    Prod =
      ...
      "-" +
      Math.floor(Products[i].y + 200) +
      ",";
  }
  console.log("yes", Prod);
  ...
  if (androidTV == true) {
    ...
    for (i = 0; i < Products.length; i++) {
      ...
      if (Products[i].Url != null) {
        ...
        new QRCode(
          ...
          "http://showtagenabled.com/" + Products[i].ProductId
        );
      }
    }
    ...
    for (var i = 0; i < ProductTag.length; i++) {
      ...
      ProductTag[i].onclick = function () {
        ...
        console.log(ProductTagContainerId);

        document.getElementById(ProductTagContainerId).style.display = "block";
      };
    }
  }
...
function CustomProductListing(settings, ProductDataCustom) {
  ...
  if (androidTV == true) {
    ...
    // we need to wait for the div's to be present in the dom before we can append
    for (i = 0; i < ProductDataCustom.length; i++) {
      console.log(ProductDataCustom[i].Url);
